...
class Pager{
	...
	private function genCustomerURL( $curl, $param, $value ){
		$valueArray = $this->valueArray;
		$valueArray[$param] = $value;
		$q_pos = strpos($curl,'?');
		if ($q_pos > 0) {
			return $curl . '&' . http_build_query($valueArray);
		} else {
			return $curl . '?' . http_build_query($valueArray);
		}
	}
	...
	public function GenPathBasic($curl) {
		$r = $this->paginate();
		$buffer = null;
		$index = '首页';
		$pre = '上一页';
		...
		$buffer .= "<li>({$this->rowCount})</li>";
		if ($this->pageNo > 1) {
			$buffer .= "<li><a href='".$this->genCustomerURL($curl, $this->pageString,1)."'>{$index}</a><li><a href='".$this->genCustomerURL($curl, $this->pageString,$this->pageNo-1)."'>{$pre}</a>";
		}
		foreach($range AS $one) {
			if ( $one == $this->pageNo ) {
				$buffer .= "<li class=\"current\">{$one}</li>";
			} else {
				$buffer .= "<li><a href='".$this->genCustomerURL($curl, $this->pageString,$one)."'>{$one}</a><li>";
			}
		}
		if ($this->pageNo < $this->pageCount) {
			$buffer .= "<li><a href='".$this->genCustomerURL($curl, $this->pageString,$this->pageNo+1)."'>{$next}</a></li><li><a href='".$this->genCustomerURL($curl, $this->pageString, $this->pageCount)."'>{$last}</a></li>";
		}
		return $buffer . '</ul>';
	}
}
