...
// 商品二级分类
	static public function OptionGroup($a=array(), $a2, $v=null, $all=null)
    {
        $option = null;
        if ( $all ){
            $selected = ($v) ? null : 'selected';
        ...
        foreach( $a AS $key=>$value )
        {
			...
			foreach( $a2[$key] AS $key2=>$value2 )
			{
				
				if (is_array($value2)) { 
					$key2 = strval($value2['id']);
					$value2 = strval($value2['name']); 
				}
				$selected2 = in_array($key2, $v) ? 'selected' : null;
				$option .= "<option value='{$key2}' {$selected2}>--".strip_tags($value2)."</option>";	
			}
        }
